...
def save_info_to_Joblib(csvpath,cantDocs):
    ...
    output_filepath = os.path.join('./', './vect/', f'{base_filename[len(csv_directory):]}_vect.joblib')
    joblib.dump(tfidf_vectorizer, output_filepath)
    output_filepath = os.path.join('./', './matrix/', f'{base_filename[len(csv_directory):]}_matrix.joblib')
    joblib.dump(tfidf_matrix,output_filepath)
